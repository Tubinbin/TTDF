...
n=4160, d=1024, p=9419;
w=m=320;

q=50496997519, N=4, T=3;
int eta2=1;
for (i=1;i<=N;i++)
	{
		eta2=eta2*i;
...
tend=clock();
double t_1;
t_1=double(tend-tbegin)/CLOCKS_PER_SEC;//generate master sk and shared sk;
...
for (i=0;i<n;i++)
	{
		for (j=0;j<m;j++)
			{
				E[i][j]=conv<ZZ_p>(ZZ(50496997519*distribution(generator)+0.5));
			}
	}
...
for(i=0;i<w;i++)
	{
		ee0[0][i]=conv<ZZ_p>(ZZ(50496997519*distribution(generator)+0.5));
	}
cout <<"ee0="<<ee0<<"\n";
...
for (i=0;i<w;i++)
	{
		ee1[0][i]=conv<ZZ_p>(ZZ(50496997519*distribution(generator)+0.5));
	}
cout <<"ee1="<<ee1<<"\n";
...
tend=clock();
double t_2;
t_2=double(tend-tbegin)/CLOCKS_PER_SEC;//////////////revoked ZZ0 and ZZ1; 

ee2.SetDims(1, w);
for (i=0;i<w;i++)
	{
		ee2[0][i]=conv<ZZ_p>(ZZ(50496997519*distribution(generator)+0.5));
	}
cout <<"ee2="<<ee2<<"\n";
...
for (i=0;i<w;i++)
	{
		ee3[0][i]=conv<ZZ_p>(ZZ(50496997519*distribution(generator)+0.5));
	}
cout <<"ee3="<<ee3<<"\n";
...
for (i=0;i<w;i++)
	{
		ee4[0][i]=conv<ZZ_p>(ZZ(50496997519*distribution(generator)+0.5));
	}
cout <<"ee4="<<ee4<<"\n";
...
cout<<"generate master sk and shared sk: t_1="<<t_1<<"\n";//seconds
cout<<"generate ciphertext to revoke user1(ZZ0) and user2(ZZ1): t_2="<<t_2<<"\n";
cout<<"non-revoked users decrypt shared sk: t_3="<<t_3<<"\n";
//*******************************
